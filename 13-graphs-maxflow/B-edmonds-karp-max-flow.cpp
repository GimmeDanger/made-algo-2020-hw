...
void solve(istream &is, ostream &os) {
    ...
    // find max flow
    int s = 0, t = n - 1;
    os << edmonds_karp_max_flow(graph, s, t) << endl;
    ...
    auto get_reverse_edge = [&graph](int input_id) -> const flow_edge & {
        int graph_id = (input_id * NETWORK_EDGES_PER_INPUT_UNDIRECT_EDGE) + NETWORK_EDGES_PER_INPUT_DIRECT_EDGE;
        return graph.get_edge(graph_id);
    };
    for (int i = 0; i < m; i++) {
        os << get_direct_edge(i).flow() - get_reverse_edge(i).flow() << endl;
    }
}
