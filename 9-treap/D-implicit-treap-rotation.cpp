...
class TreapArray {
    static constexpr int SUBTREE_INITIAL_SIZE_DEFAULT = 1;
    static constexpr bool SUBTREE_NEED_ROTATION_DEFAULT = false;
    ...
    struct Node {
        ...
        explicit Node(int value_, int priority_, Node *left_ = nullptr, Node *right_ = nullptr)
                : value(value_), priority(priority_), size(SUBTREE_INITIAL_SIZE_DEFAULT),
                  need_rotation(SUBTREE_NEED_ROTATION_DEFAULT), left(left_), right(right_) {}
    };
    ...
    void set_rotation_mark(Node *node) {
        if (node) {
            node->need_rotation ^= true;
        }
    }

    void push(Node *node) {
        if (node && node->need_rotation) {
            swap(node->left, node->right);
            node->need_rotation = false;
            set_rotation_mark(node->left);
            set_rotation_mark(node->right);
        }
    }
    ...
    // Complexity: O(h)
    void to_vec_impl(Node *node, vector<int> &res) {
        push(node);
        if (node) {
