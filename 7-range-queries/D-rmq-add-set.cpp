...
class SegmentTree {
  ...
  ll get(size_t v, size_t l, size_t r) const {
    ...
    // }
    // For RMQ with set, add:
    if (upd_set[v].has_value()) {
      return upd_set[v].value();
    }
    else {
      return t[v] + upd_add[v];
    }
  }
  ...
  void push(size_t v, size_t l, size_t r) {
    if (l == r /*v -- leaf*/) {
      ...
      t[v] = min(get(left(v), l, m), get(right(v), m + 1, r));
    }
    upd_add[v] = 0;
    upd_set[v] = {};
  }
  ...
  ll rmq_impl(size_t v, size_t l, size_t r, size_t a, size_t b) {
    push(v, l, r);
    if (l > b || r < a) {
    ...
    if (l >= a && r <= b)
      return get(v, l, r);
    size_t m = l + (r - l) / 2;
    return min(rmq_impl(left(v), l, m, a, b),
               rmq_impl(right(v), m + 1, r, a, b));
  }
 ...
 public:
  ...
  // Compute min on segment [l, r]
  // Complexity: O(log n)
  ll rmq(size_t l, size_t r) {
    return rmq_impl(0, 0, x - 1, l, r);
  }
...
void process_queries(istream &is, ostream &os, SegmentTree &st, SegmentTreeNaive &stn) {
  ...
  while (is >> command) {
    ...
    if (command == "set") {
      ...
      is >> i >> j;
      auto res = st.rmq(i - 1, j - 1);
      if (dbg) {
        auto res_naive = stn.rmq(i - 1, j - 1);
...
void fuzzy_testing() {
  ...
  for (int t = 0; t < TESTS; t++) {
    ...
    std::uniform_int_distribution<int> l_distr (0, a.size()-1);
    vector<string> queries;

    for (int q = 0; q < QUERIES; q++) {
      stringstream is, os;
      int q_type = q_distr(gen); // add, set or rsq
      auto l = l_distr(gen);
